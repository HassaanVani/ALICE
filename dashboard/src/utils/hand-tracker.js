 ...
 * Requires MediaPipe scripts loaded in index.html:
...
export class HandTracker {
  ...
  async initialize() {
    ...
    if (!Hands || !Camera) {
      throw new Error('MediaPipe not loaded — check script tags in index.html');
    }
  ...
  drawHand(landmarks) {
    ...
    landmarks.forEach((lm, i) => {
      ...
      this.ctx.fillStyle = isFingertip ? '#8b5cf6' : '#7c5cff';
      this.ctx.fill();
    });
  }
}
