...
| CNN (`brain/`) | Block classification | Desk object recognition |
| ArUco detection (`vision/aruco_detector.py`) | Workspace marker tracking | Workspace mapping + presence zones |
| Kalman tracker (`vision/tracker.py`) | Block position tracking | Object position tracking |
| Camera pipeline (`vision/camera.py`) | Frame capture | Same, add motion detection |
| Arm controller (`hardware/arm_controller.py`) | Direct joint control | Same, wrapped by movement dynamics |
...
| Tensor server (`server.py`) | CNN activation streaming | Glass brain feed for product UI |
...
| Haptix (hand tracking) | Puppeteer input | Teaching mode — show ALICE new routines |
| Dashboard (React/Three.js) | Debug/visualization tool | Consumer product interface |
| State manager (`state.py`) | Internal state sync | Same, extended with personality state |
| Config system (`config.py`) | YAML/env/CLI config | Same, add personality params |
| Self-test (`selftest.py`) | Startup health check | Same |
...
- Camera-based motion detection for wake/sleep transitions
- Simple threshold on frame differencing in the overhead camera
- Triggers idle → active transition with personality-appropriate delay
- Lives in `vision/` as a new module

#### 2. Object Memory
- Persistent storage of recognized objects + preferred positions
- Builds over sessions — ALICE remembers where things were last time
- Tracks user overrides to update preferences
- JSON store, loaded at startup
- New module in `brain/` or `logic/`
...
- **Spatial Preferences (left, lower):** Minimap of the desk with preferred object positions. Shows where ALICE *thinks* things should be vs. where they are.
...
- [ ] Presence detection module (`vision/presence.py`)
- [ ] Object memory persistence (`brain/object_memory.py` or `logic/`)
- [ ] Desk layout presets in config
- [ ] Personality engine — preference model, resistance thresholds
- [ ] Movement dynamics wrapper (`hardware/dynamics.py`)
- [ ] Expand CNN training for desk objects (or use zero-shot/few-shot approach)
...
- [ ] Coffee interaction choreography (partially scripted, partially emergent)
...
- [ ] Object memory panel, spatial preferences minimap
...
1. **CNN retraining scope:** Do we retrain for real desk objects, use a pre-trained object detector (YOLO/etc.), or stay with ArUco markers as proxies and let the narration sell the concept?

2. **Voice output modality:** Text on dashboard only? TTS through speaker? Both? TTS adds presence but risks breaking the "she speaks rarely" principle if the voice isn't right.

3. **Choreography vs. emergence:** How much of the demo should be scripted vs. driven by the personality engine? More scripted = reliable but rigid. More emergent = alive but risky for live demo.

4. **Audience interaction platform:** Keep WebSocket-based? Switch to something more accessible (QR code → mobile web page → vote)?

5. **Branding depth:** Is "ALICE" the final name or does the Detroit: Become Human connection need to be obscured for IP reasons? Does she need a logo, a typeface, a visual identity beyond the dashboard?
...
- Someone in the audience pulls out their phone to record the coffee moment
